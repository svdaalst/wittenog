...
public record RenameFlowCommand(FlowDiagram Flow, string NewStem) : IRequest<FlowDiagram>;

public class RenameFlowCommandHandler : IRequestHandler<RenameFlowCommand, FlowDiagram>
{
    ...
    public RenameFlowCommandHandler(IFlowRepository repo, IWikiLinkParser linkParser)
    {
        _repo       = repo;
        _linkParser = linkParser;
    }

    public async Task<FlowDiagram> Handle(RenameFlowCommand request, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Public so <see cref="WitteNog.App.Components.FlowBlock"/> can pre-validate
    /// before sending the command (same pattern as <see cref="RenameDrawingCommandHandler.ValidateStem"/>).
    /// </summary>
